...
/**
Класс свойств объектов, описывает различные параметры, характеристики и своства
различных объектов Свойство содержит словарь double значени с соответствующими
имена и\или строку - значение. Например: свойство "координаты объекта" содержит
словарь значений (1, 1, 1) с именами ("x", "y", "z")
*/
class IProperties {
  public:
    ...
    /// Задает строковое значение свойства, после чего свойство становится
    /// строковым
    virtual void SetStringValue(std::string val) = 0;
};

///Класс с базовой реализациейинтерфейса свойств объектов, описывает параметры
///различных объектов
class TProperties : public IProperties {
  ...
  public:
    ...
    TProperties(std::map<std::string, double> _map, bool _isObserved = false,
                std::string _name = "");
    TProperties(const TProperties &properties);

    /// Возвращает словарь свойства
    virtual std::map<std::string, double> &GetValues() override {
        return propMap;
    }
    /// Задает словарь для свойства
    virtual void SetValues(const std::map<std::string, double> &_map) override {
        propMap = _map;
    }
    ...
    /// Задает наблюдается ли это свойство стандартным сенсором
    virtual void SetIsObserved(bool _isObserved) override {
